...
@Controller
@RequestMapping("/switchMonitor")
public class SwitchMonitorController {
	
	@Autowired
	private SwitchMonitorService switchMonitorService;

	@RequestMapping({"","/"})
	public String dbmonitor(String vague, Model model) {
		model.addAttribute("configs", switchMonitorService.findConfig(vague));
	...
	@ResponseBody
	public String saveHostConfig(SwitchMonitorConfig config) {
		switchMonitorService.saveConfig(config);
//		return "SUCCESS";
		return "操作成功";
	}
	...
	@ResponseBody
	public SwitchMonitorConfig findConfigById(@PathVariable("id")String id) {
		return switchMonitorService.findConfigById(id);
	}
	
	@RequestMapping("/config_del/{id}")
	@ResponseBody
	public String del(@PathVariable("id")String id) {
		switchMonitorService.deleteConfig(id);
//		return "SUCCESS";
		return "操作成功";
	}
	...
	public String startMonitor(@PathVariable("id")String id) {
		switchMonitorService.startMonitor(id);
//		return "SUCCESS";
		return "操作成功";
	}
	...
	public String stopMonitor(@PathVariable("id")String id) {
		switchMonitorService.stopMonitor(id);
//		return "SUCCESS";
		return "操作成功";
	}
	...
	public String startMonitorBatch(@PathVariable("ids")String ids) {
		...
		int successCount = 0;
		for(String id : idArr) {
			boolean success = switchMonitorService.startMonitor(id);
			if(success) {
				successCount ++;
			}
		}
	...
	public String stopMonitorBatch(@PathVariable("ids")String ids) {
		...
		int successCount = 0;
		for(String id : idArr) {
			boolean success = switchMonitorService.stopMonitor(id);
			if(success) {
				successCount ++;
			}
		}
	...
	public String switchMonitor(@PathVariable("id")String id) {
		switchMonitorService.switchMonitor(id);
//		return "SUCCESS";
		return "操作成功";
	}
	...
	public boolean doMonitor(@PathVariable("id")String id) {
		return switchMonitorService.monitor(id);
	}
	...
	public String setBind(@PathVariable("configId")String configId, @PathVariable("equipmentId")String equipmentId) {
		switchMonitorService.saveBind(configId, equipmentId);
//		return "SUCCESS";
		return "操作成功";
	}
}
