...
function AgendaView(element, calendar, viewName) {
	...
	/* Scrolling
	-----------------------------------------------------------------------*/
	function resetScroll(timeStr) {
		//alert(opt('scrollTime'));
		/*
		...
		*/
		if(timeStr==null) {
			timeStr = opt('scrollTime');
		}
		//alert(timeStr);
		var top = computeTimeTop(
				moment.duration(timeStr)
			) + 1; // +1 for the border
	...
	function afterRender(timeStr) { // after the view has been freshly rendered and sized
		resetScroll(timeStr);
	}
