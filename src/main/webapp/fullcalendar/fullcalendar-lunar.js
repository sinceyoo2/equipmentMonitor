...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		//在月视图加入农历日期及节气等
		var cld = null;
		if(cldObj[date.year()+"-"+date.month()] !=null){
			cld = cldObj[date.year()+"-"+date.month()];
		} else {
			cld = new calendar_l(date.year(), date.month());
			cldObj[date.year()+"-"+date.month()] = cld;		
		}
		var cldElt = cld[date.date()-1];
		var lday = cldElt.lDay;
		var lday_cn = cDay(lday);
		var lmonth = cldElt.lMonth;
		var lmonth_cn = monthNameCN[lmonth-1];
		//lmonth_cn=lmonth+lmonth_cn;
		var l_cn = "<span style='float:right;padding-right:3px;font-size:12px;color:#808080'>"+lmonth_cn+lday_cn+"</span>";
		var gljr = cldElt.solarTerms+(sFtvObj[date.month()+"-"+date.date()]==null?"&nbsp;":sFtvObj[date.month()+"-"+date.date()]);
		var nljr = lFtvObj[lmonth+"-"+lday]==null?"&nbsp;":lFtvObj[lmonth+"-"+lday];
		var jq = "<br/><span style='float:right;padding-right:3px;font-size:12px;color:#808080'>"+nljr+"</span><span style='font-size:12px;width:auto'>"+gljr+"</span>";
		//alert(cldElt.lunarFestival); //农历节日
		//alert(cldElt.solarFestival); //国历节日
		//alert(cldElt.solarTerms); //节气
		//在月视图加入农历日期及节气等
		...
		if (showNumbers) {
			html += "<div class='fc-day-number'>" + l_cn + date.date() + jq + "</div>";
	...
	function buildDayTableHeadHTML() {
		...
		for (col=0; col<colCnt; col++) {
			...
			*/
			...
			var gljr = cldElt.solarTerms+(sFtvObj[date.month()+"-"+date.date()]==null?"&nbsp;":sFtvObj[date.month()+"-"+date.date()]);
			var nljr = lFtvObj[lmonth+"-"+lday]==null?"&nbsp;":lFtvObj[lmonth+"-"+lday];
			var jq = "<span style='font-size:12px;color:#808080;line-height:16px'>"+nljr+gljr+"</span><br/>";
			//alert(cldElt.lunarFestival); //农历节日
			//alert(cldElt.solarFestival); //国历节日
			//alert(cldElt.solarTerms); //节气
			html +=
				"<th class='fc-" + dayIDs[date.day()] + " fc-col" + col + ' ' + headerClass + "'>" + jq +
				htmlEscape(formatDate(date, colFormat)) +
				"</th>";
